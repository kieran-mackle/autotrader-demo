...
class Rebalance:
    ...
    def generate_signal(self, i):
        ...
        signal_dict = {"order_type": "market", "direction": 0}
        ...
        if all(inst in current_holdings.keys() for inst in rebalance_instruments):
            ...
            if days_since_last_rebalance >= self.params["rebalance_every_N_days"]:
                ...
                allocation_error = abs(
                    100 * asset_allocation[self.instrument] / total_value
                ...
                if allocation_error > self.params["rebalance_pc_tolerance"]:
                    ...
                    size_difference = (
                        required_size - current_holdings[self.instrument].long_units
                    )

                    # Place order using calculated size difference to rebalance
                    signal_dict["direction"] = np.sign(size_difference)
                    signal_dict["size"] = self.check_margin_requirements(
                        self.data.Close[i], size_difference
                    )
            ...
            if self.instrument not in current_holdings:
                ...
                signal_dict["size"] = self.check_margin_requirements(
                    self.data.Close[i], nominal_size
                )
    ...
    def calculate_position_size(self, price):
        ...
        instrument_allocation_pc = (
            self.params["rebalance_percentages"][self.instrument] / 100
        )
        instrument_allocation_value = (
            instrument_allocation_pc * account_balance * self.params["account_leverage"]
        )
        position_size = (
            np.floor(
